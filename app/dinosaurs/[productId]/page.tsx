...
async function getDinoData(id: string) {
  try {
    const res = await fetch(`http://flask-env.eba-muajupmt.eu-north-1.elasticbeanstalk.com/products/${id}`, {
...
async function getReviewsData(productId: string) {
  try {
    const res = await fetch(`http://flask-env.eba-muajupmt.eu-north-1.elasticbeanstalk.com/review/${productId}`, {
      cache: 'no-store' // Disabled cache for development
    });
